...
import com.freegang.androidtemplate.base.recycler.AdapterExpandNotify;
...
public class MultipleChoiceListAdapter extends BaseAdapter<MultipleChoiceItem, MultipleChoiceListAdapter.ViewHolder> {
    private int[] selected;  //默认已选择的项, 下标数组, 如果某一个下标不存在, 则不做任何操作.
    private OnSelectedListener onSelectedListener; //选中项改变状态响应事件
    ...
    public void callBindViewHolder(@NonNull ViewHolder holder, @NonNull MultipleChoiceItem item, int position) {
        ...
        if (selected != null && selected.length != 0) {
            for (int sel : selected) {
                ...
                getDataList().get(sel).setSelected(true);
                holder.multipleChoiceItemIcon.setVisibility(getDataList().get(sel).getSelected() ? View.VISIBLE : View.INVISIBLE);
            }
    ...
    public CurrentMultipleChoiceItems getSelected() {
        ...
        //遍历
        List<MultipleChoiceItem> dataList = getDataList();
    ...
    public static class CurrentMultipleChoiceItems {
        public final AdapterExpandNotify notify;
        public final ArrayList<MultipleChoiceItem> choiceItems;
        public final ArrayList<Integer> choiceIndexes;

        public CurrentMultipleChoiceItems(AdapterExpandNotify notify, ArrayList<MultipleChoiceItem> choiceItems, ArrayList<Integer> choiceIndexes) {
            this.notify = notify;
    ...
    public interface OnSelectedListener {
        void onSelected(AdapterExpandNotify notify, MultipleChoiceItem item, int position);
    }
}
