...
class ListActivity : BaseActivity<ActivityListBinding>() {
    ...
    override fun initEvent(binding: ActivityListBinding) {
        ...
        binding.smoothToTop.setOnClickListener {
            val controller = itemAdapter.smoothScrollToTop()
            GlobalScope.launch {
                delay(1000)
                controller.stopScroll()
                runOnUiThread { showToast("终止滚动") }
            }
        }

        //滑动到列表底部
        binding.smoothToBottom.setOnClickListener {
            val controller = itemAdapter.smoothScrollToBottom()
        }
        ...
        itemAdapter.setOnItemClickListener { adapter, preItem, nextItem, item, position ->
            showToast("点击事件: $preItem, $nextItem, $item, $position")
        }

        itemAdapter.setOnItemClickListener { adapter, preItem, nextItem, item, position ->
            showToast("长按事件: $preItem, $nextItem, $item, $position")
            true
        }
    }
}